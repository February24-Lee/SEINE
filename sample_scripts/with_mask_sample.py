...
def auto_inpainting(
    ...
    mask = torch.nn.functional.interpolate(
        mask[:, :, 0, :], size=(args.latent_h, args.latent_w)
    ).unsqueeze(1)
...
def main(args):
    ...
    args.latent_h = args.image_size[0] // 8
    args.latent_w = args.image_size[1] // 8
    print("loading model")
    ...
    # load model
    state_dict = torch.load(args.ckpt, map_location=lambda storage, loc: storage)["ema"]
